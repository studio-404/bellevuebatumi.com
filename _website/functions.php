...
function slide(){
	$out  = '';	
	$slides = db_fetch_all("SELECT * FROM " . c("table.galleries") . " WHERE  language = '" . l() . "' AND deleted=0 AND menuid=5 AND visibility = 1" );
	
	if( $slides ) {
		foreach($slides as $gal) {
			$out.='
					<div class="HomeBackground BackgroundStyle" style="background:url('.$gal["image1"].');"></div>
					';
		}
	}
    return $out;
...
//		menu-selected
		...
		$out .= '<li'.(($sections[$idx]["id"] == $active && $sections[$idx]["id"] != 1) ? ' class="active"':'').'>'."\n";
		    $sql_in = "SELECT id, title, idx, redirectlink, menutitle, category, masterid FROM " . c("table.pages") . " WHERE language = '" . l() . "' AND menuid = 11 AND deleted = 0 AND masterid = " . $sections[$idx]['id'] . " AND visibility = 1 ORDER BY position;";
